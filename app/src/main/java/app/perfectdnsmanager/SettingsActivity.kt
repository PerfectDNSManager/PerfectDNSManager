...
import app.perfectdnsmanager.util.pdmAccentSupport
import app.perfectdnsmanager.util.pdmDanger
import app.perfectdnsmanager.util.pdmWarning
...
import app.perfectdnsmanager.data.ProfileManager
...
class SettingsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        rowTheme.setOnClickListener {
            ...
            app.perfectdnsmanager.util.TvDialog.showRadioPicker(
                this,
            ...
            if (profileJson != null) {
                try {
                    ...
                    val typeLabel = when (profile.type) {
                        app.perfectdnsmanager.data.DnsType.DOH -> "DoH"
                        app.perfectdnsmanager.data.DnsType.DOT -> "DoT"
                        app.perfectdnsmanager.data.DnsType.DOQ -> "DoQ"
                        app.perfectdnsmanager.data.DnsType.DEFAULT -> "Standard"
                    }
                    val methodLabel = if (profile.type == app.perfectdnsmanager.data.DnsType.DOT) "ADB" else "VPN"
        ...
        switchAdvanced.setOnCheckedChangeListener { _, isChecked ->
            ...
            if (!isChecked) {
                ...
                val repo = app.perfectdnsmanager.data.DnsRewriteRepository(this)
        ...
        findViewById<Button>(R.id.btnResetApp).setOnClickListener {
            AlertDialog.Builder(this)
                ...
                .setPositiveButton(getString(R.string.reset_app_confirm)) { _, _ ->
                    try {
                        val stopIntent = Intent(this, app.perfectdnsmanager.service.DnsVpnService::class.java)
        ...
        val updateManager = app.perfectdnsmanager.service.UpdateManager(this)
        ...
        btnCheck.setOnClickListener {
            ...
            updateManager.checkForUpdateGitHub("PerfectDNSManager/PerfectDNSManager", BuildConfig.VERSION_NAME)
        }
    ...
    private fun showUrlRewriteDialog() {
        ...
        app.perfectdnsmanager.util.TvDialog.showMenuPicker(
            this,
    ...
    private fun showAddRewriteRuleDialog(repo: app.perfectdnsmanager.data.DnsRewriteRepository) {
    ...
    private fun showEditRewriteRuleDialog(repo: app.perfectdnsmanager.data.DnsRewriteRepository, rule: app.perfectdnsmanager.data.DnsRewriteRule) {
    ...
    private fun reloadVpnRewriteRules() {
        try {
            val intent = android.content.Intent(this, app.perfectdnsmanager.service.DnsVpnService::class.java)
            intent.action = app.perfectdnsmanager.service.DnsVpnService.ACTION_RELOAD_RULES
            startService(intent)
        } catch (_: Exception) { }
    }
    ...
    private fun encryptAndUpload(content: String, expiresIn: String = "1h") {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                val result = app.perfectdnsmanager.util.EncryptedSharer.encryptAndUpload(
    ...
    private fun showImportFromCodeDialog() {
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton(getString(R.string.download_button)) { _, _ ->
                ...
                lifecycleScope.launch(Dispatchers.IO) {
                    try {
                        val json = app.perfectdnsmanager.util.EncryptedSharer.downloadAndDecrypt(this@SettingsActivity, code, pwd)
                        if (json.isNotBlank()) {
    ...
    private fun showImportResult(result: app.perfectdnsmanager.data.ConfigManager.ImportResult) {
